...
def plot_sine_approximation(
    ...
    axes.scatter(
        X_train[:, 0],
        model_on_training_data,
        label="Fitted model on training data",
        linestyle="dashed",
        color="red",
    )
...
def my_plot(
    ...
    plot_sine_approximation(
        axes,
        x_values,
        training_X,
        training_y,
        y_values,  # Fitted model
        predictions_on_training,  # predictions on training data
        validation,
        validation_y,
        nodes,  # filename
    )
