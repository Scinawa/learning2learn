...
class NoisySinDataModule(LightningDataModule):
    def __init__(self, interval=2, dataset_size=1000, validation_ratio=0.2, batch_size=32, noise_of_noisy_feature=0.0, how="linspace"):
        super().__init__()
    ...
    def prepare_data(self):
        ...
        if self.how == "linspace":
            x = np.linspace(-self.interval * np.pi, self.interval * np.pi, self.dataset_size)
        elif self.how == "random":
            x = np.random.uniform(-self.interval * np.pi, self.interval * np.pi, self.dataset_size)
        else:
        ...
        y = np.sin(x) + np.random.normal(loc=0.0, scale=self.noise_of_noisy_feature, size=x.shape)
    ...
    def setup(self, stage=None):
        ...
        self.train_dataset, self.val_dataset = random_split(dataset, [training_set_size, validation_set_size])
